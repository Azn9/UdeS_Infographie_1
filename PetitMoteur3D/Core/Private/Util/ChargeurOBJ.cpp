...
template <class T> bool Extraire(std::istringstream& iss, T& f1)
{
	return !(iss >> std::dec >> f1).fail();
}

template <class T> bool Extraire(std::istringstream& iss, T& f1, T& f2)
{
	return !(iss >> std::dec >> f1 >> std::dec >> f2).fail();
}

template <class T> bool Extraire(std::istringstream& iss, T& f1, T& f2, T& f3)
{
	return !(iss >> std::dec >> f1 >> std::dec >> f2 >> std::dec >> f3).fail();
}

template <class T> void ExtraireSommet(const std::string& s, T& f1, T& f2, T& f3, bool bTC, bool bN)
{
	std::istringstream iss(s);
	iss >> std::dec >> f1;
	if (bTC || bN)
