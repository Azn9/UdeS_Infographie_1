...
D3D11_INPUT_ELEMENT_DESC TextShaderLayout[] =
{
    {
...
UINT TextShaderNumElements;

PM3D::CSommetSprite textSommets[6] = {
...
PM3D_API::TextShader::TextShader(const std::wstring& fileName)
    : Shader(), vertexBuffer(nullptr), indexBuffer(nullptr)
{
    ...
    bd.ByteWidth = sizeof(textSommets);
    bd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bd.CPUAccessFlags = 0;

    D3D11_SUBRESOURCE_DATA InitData;
    ZeroMemory(&InitData, sizeof(InitData));
    InitData.pSysMem = textSommets;
    ...
    shaderParametersBufferDesc.ByteWidth = sizeof(TextShaderParameters);
...
#pragma warning(pop)
    ...
    TextShaderNumElements = ARRAYSIZE(TextShaderLayout);

    PM3D::DXEssayer(pD3DDevice->CreateInputLayout(
                        TextShaderLayout,
                        TextShaderNumElements,
                        vsCodePtr,
                        vsCodeLen,
                        &vertexLayout),
                    DXE_CREATIONLAYOUT);
...
PM3D_API::TextShader::~TextShader()
{
...
void PM3D_API::TextShader::LoadLights(ID3D11DeviceContext* context, GameObject* gameObject)
{
    // No lights for sprites
}

void* PM3D_API::TextShader::PrepareParameters(DirectX::XMMATRIX matWorldViewProj, DirectX::XMMATRIX matWorld)
{
    ...
    const auto parameters = new TextShaderParameters{
        matWorldViewProj
    };
...
void PM3D_API::TextShader::ApplyMaterialParameters(
    ...
    ID3D11ShaderResourceView* normalmapTexture)
{
    ...
    {
        throw std::runtime_error("Trying to use TextShader without a texture!");
    }
...
void PM3D_API::TextShader::DeleteParameters(void* shader_parameters)
{
    delete static_cast<TextShaderParameters*>(shader_parameters);
}
