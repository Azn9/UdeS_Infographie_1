...
PM3D_API::FontLoader::FontLoader(const std::string& fileName)
{
	std::string baseName;

	if (fileName.find_last_of(".fnt") == std::string::npos)
		baseName = fileName;
	else
		baseName = fileName.substr(0, fileName.find_last_of(".fnt"));

	const auto fntFile = baseName + ".fnt";
	const auto ddsFile = baseName + ".dds";

	std::ifstream fntStream(fntFile);

	if (!fntStream.is_open())
		throw std::runtime_error("Could not open " + fntFile);
	...
	for (std::string line; getline(fntStream, line);)
	{
		if (line.find("char ") == 0)
		{
			...
			const auto height = std::stoi(data.substr(data.find("height=") + 7, data.find(' ', data.find("height=")) - 7));
			const auto xoffset = std::stoi(data.substr(data.find("xoffset=") + 8, data.find(' ', data.find("xoffset=")) - 8));
			const auto yoffset = std::stoi(data.substr(data.find("yoffset=") + 8, data.find(' ', data.find("yoffset=")) - 8));
			...
			const auto letter = data.substr(data.find("letter=\"") + 8, data.find('\"', data.find("letter=\"") + 9) - 8);
