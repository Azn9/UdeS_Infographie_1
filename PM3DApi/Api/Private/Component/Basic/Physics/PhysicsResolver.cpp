...
void PM3D_API::PhysicsResolver::Initialize()
{
    ...
    if (physx::PxPvdTransport* mTransport = physx::PxDefaultPvdSocketTransportCreate("localhost", 5425, 10000);
        mTransport !=
        nullptr)
    {
...
void PM3D_API::PhysicsResolver::InitializeScene()
{
    ...
    dispatcher = physx::PxDefaultCpuDispatcherCreate(6);
    sceneDesc.cpuDispatcher = dispatcher;
    sceneDesc.filterShader = FilterShader;

    const auto simulationCallback = new SimulationCallback;
...
void PM3D_API::PhysicsResolver::ResolvePhysics()
{
    ...
    for (const auto& gameObject : parentObject->GetScene()->GetChildren())
    {
        if (const auto rigidbody = gameObject->GetComponent<Rigidbody>())
        {
            ...
            if (const physx::PxRigidDynamic* dynamic = static_cast<physx::PxRigidDynamic*>(actor); dynamic->
                isSleeping())
                continue;
