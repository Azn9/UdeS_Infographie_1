...
void Util::DrawDebugVector3(const std::string name, const DirectX::XMVECTOR& vec)
{
    ...
    ImGui::SameLine(100);
    ImGui::Text(("x=" + std::to_string(vec.m128_f32[0])).c_str());
    ImGui::SameLine(200);
    ImGui::Text(("y=" + std::to_string(vec.m128_f32[1])).c_str());
    ImGui::SameLine(300);
    ImGui::Text(("z=" + std::to_string(vec.m128_f32[2])).c_str());
}
...
{
    float x = asinf(direction.z / DirectX::XMVectorGetX(DirectX::XMVector3Length(XMLoadFloat3(&direction)))) * 180.0f /
        DirectX::XM_PI;
