...
namespace PM3D
{
    ...
    void CMoteurWindows::SetWindowsAppInstance(HINSTANCE hInstance)
    {
        ...
        // Stocke le handle d'instance de l'application, plusieurs fonctions sp�cifiques en auront besoin
    }
    ...
    bool CMoteurWindows::InitAppInstance()
    {
        ...
        wc.hIcon = LoadIcon(hAppInstance, MAKEINTRESOURCE(IDI_PETITMOTEUR3D));
...
void CMoteurWindows::EndRenderDebug()
{
    ...
    LRESULT CALLBACK CMoteurWindows::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
    {
        ...
        case WM_SETFOCUS:
            ...
            break;
        case WM_MOUSEMOVE:
            Input::GetInstance().SetMousePosition(LOWORD(lParam), HIWORD(lParam));
            break;
        case WM_LBUTTONDOWN:
            Input::GetInstance().SetMouseLeftPressed(true);
            break;
        case WM_LBUTTONUP:
            Input::GetInstance().SetMouseLeftPressed(false);
            break;
        case WM_RBUTTONDOWN:
            Input::GetInstance().SetMouseRightPressed(true);
            break;
        case WM_RBUTTONUP:
