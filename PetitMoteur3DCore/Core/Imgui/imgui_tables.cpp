...
void ImGui::TableAngledHeadersRowEx(float angle, float max_label_width)
{
    ...
    const float row_height = ImFabs(
        ImRotate(ImVec2(max_label_width, flip_label ? +header_height : -header_height), cos_a, sin_a).y);
        ...
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            if (pass == 0)
            {
                ...
                ImRect label_r(window->ClipRect.Min,
                               window->ClipRect.Min + ImVec2(clip_width + (flip_label ? 0.0f : table->CellPaddingX),
                ...
                RenderTextEllipsis(draw_list, label_r.Min + label_off, label_r.Max, label_r.Max.x, label_r.Max.x,
