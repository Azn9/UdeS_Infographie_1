...
D3D11_INPUT_ELEMENT_DESC snowShaderLayout[] =
{
    {
...
void SnowShader::Initialize(const std::wstring& wstring)
{
    ...
    shaderParametersBufferDesc.ByteWidth = sizeof(SnowShaderParameters);
    ...
    if (res != S_OK)
    {
        ...
        throw std::runtime_error("Shader compilation failed! (SnowShader > " + convertedFileName + ")");
    }
    ...
    if (res != S_OK)
    {
        throw std::runtime_error("Shader creation failed! (SnowShader > " + convertedFileName + ")");
    }
...

    {
        ...
        snowShaderNumElements = ARRAYSIZE(snowShaderLayout);

        PM3D::DXEssayer(pD3DDevice->CreateInputLayout(snowShaderLayout,
                                                      snowShaderNumElements,
                                                      vsCodePtr,
                                                      vsCodeLen,
                                                      &vertexLayoutShadow),
                        DXE_CREATIONLAYOUT);
...
void* SnowShader::PrepareParameters(
...
)
{
    ...
    const auto parameters = new SnowShaderParameters{
        matWorldViewProj,
        matWorld,
...
void SnowShader::ApplyMaterialParameters(
...
void SnowShader::LoadLights(ID3D11DeviceContext* context, PM3D_API::GameObject* gameObject)
{
    ...
    lightParametersBufferDesc.ByteWidth = sizeof(PM3D_API::ShaderLightDefaultParameters) * shaderLightsParameters.size();
