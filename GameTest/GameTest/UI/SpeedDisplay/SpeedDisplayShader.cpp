...
D3D11_INPUT_ELEMENT_DESC speedDisplayShaderLayout[] =
{
    {
...
UINT speedDisplayShaderNumElements;

PM3D::CSommetSprite speedDisplaySommets[6] = {
...
PM3D_API::SpeedDisplayShader::SpeedDisplayShader(const std::wstring& fileName) : Shader(), vertexBuffer(nullptr),
    indexBuffer(nullptr)
{
    ...
    bd.ByteWidth = sizeof(speedDisplaySommets);
    bd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bd.CPUAccessFlags = 0;

    D3D11_SUBRESOURCE_DATA InitData;
    ZeroMemory(&InitData, sizeof(InitData));
    InitData.pSysMem = speedDisplaySommets;
    ...
    shaderParametersBufferDesc.ByteWidth = sizeof(SpeedDisplayShaderParameters);
...
#pragma warning(pop)
    ...
    speedDisplayShaderNumElements = ARRAYSIZE(speedDisplayShaderLayout);

    PM3D::DXEssayer(pD3DDevice->CreateInputLayout(
                        speedDisplayShaderLayout,
                        speedDisplayShaderNumElements,
                        vsCodePtr,
                        vsCodeLen,
                        &vertexLayout),
                    DXE_CREATIONLAYOUT);
...
PM3D_API::SpeedDisplayShader::~SpeedDisplayShader()
{
...
void PM3D_API::SpeedDisplayShader::LoadLights(ID3D11DeviceContext* context, GameObject* gameObject)
{
    // No lights for sprites
}

void* PM3D_API::SpeedDisplayShader::PrepareParameters(DirectX::XMMATRIX matWorldViewProj, DirectX::XMMATRIX matWorld)
{
    ...
    const auto parameters = new SpeedDisplayShaderParameters{
        matWorldViewProj
    };
...
void PM3D_API::SpeedDisplayShader::ApplyMaterialParameters(
    ...
    ID3D11ShaderResourceView* normalmapTexture)
{
    ...
    {
        throw std::runtime_error("Trying to use SpeedDisplayShader without a texture!");
    }
...
void PM3D_API::SpeedDisplayShader::DeleteParameters(void* shader_parameters)
{
    delete static_cast<SpeedDisplayShaderParameters*>(shader_parameters);
}
